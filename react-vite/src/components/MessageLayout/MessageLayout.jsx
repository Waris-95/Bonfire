...
import { io } from "socket.io-client";
import { useDispatch, useSelector } from "react-redux";
import { createMessageThunk, updateMessageThunk, deleteMessageThunk } from "../../redux/message";
...
export default function MessageLayout({ defaultMessages, channelId, prevChannelId }) {
    ...
    const messageElements = useMemo(() => messages.map((message) => {
        ...
        return <Message key={message.id} message={message} text={message.text_field} date={message.updated_at} name={message.user?.username} img={url} onEdit={handleEditMessage} onDelete={handleDeleteMessage} />
    }), [messages])
